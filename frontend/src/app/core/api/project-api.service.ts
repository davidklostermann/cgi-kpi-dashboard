...
export class ProjectApiService extends ApiClient {
  ...
  getProjectIsoManagement(projectId: string) {
    return this.get<ProjectIsoManagement>(`/projects/${projectId}/iso-management`);
  }

  getAgileDelivery(projectId: string) {
    return this.get<AgileDelivery>(`/projects/${projectId}/agile-delivery`);
  }
}
