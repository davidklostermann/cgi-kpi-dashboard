...
describe('ProjectTeamCapacitySectionComponent', () => {
  ...
  it('should render roles and capacity summary from the API', () => {
    ...
    expect(text).not.toContain('Gehalt');
    expect(text).not.toContain('Mitarbeiter');
    expect(fixture.nativeElement.querySelectorAll('.capacity-section__roles li').length).toBe(2);
    expect(fixture.nativeElement.querySelector('.capacity-section__summary')).toBeTruthy();
  });
