...
describe('ProjectAiPanelComponent', () => {
  ...
  it('should answer conversational chat without insufficientEvidence hint', () => {
    const fixture = createPanel();
    fixture.componentInstance.loadAnalysis(false);
    flushReadiness();
    httpMock.expectOne(analysisUrl).flush(analysis);
    ...
    fixture.componentInstance.sendQuestion('Hallo');
    fixture.detectChanges();

    httpMock.expectOne(readinessUrl).flush({ ready: true });
    const req = httpMock.expectOne(questionsUrl);
    req.flush({
      answer: 'Hallo! Wobei kann ich helfen?',
      evidenceFactIds: [],
      factsAsOf: '2026-07-01T08:00:00Z',
      generatedAt: '2026-07-16T12:00:00Z',
    ...
    const text = fixture.nativeElement.textContent as string;
    expect(text).toContain('Hallo! Wobei kann ich helfen?');
    expect(text).not.toContain('reichen für eine belastbare Antwort nicht aus');
  });
