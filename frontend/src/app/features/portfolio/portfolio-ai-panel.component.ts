...
import { Subject, catchError, map, of, switchMap, take } from 'rxjs';
...
type LoadStatus = 'loading' | 'success' | 'error' | 'disabled' | 'key_missing';
    ...
    this.reload$
      ...
      .pipe(
        ...
        switchMap(() => {
          ...
          return this.projectAiApi.checkReadiness().pipe(
            ...
            switchMap((response) => {
              ...
              if (generation !== this.loadGeneration) {
                ...
                return of({ kind: 'stale' as const });

              }

              if (response.ready !== true) {

                return of({ kind: 'key_missing' as const });

              }

              return this.aiApi.getPortfolioTrend(this.filterService.toQueryParams()).pipe(

                map((payload) => ({ kind: 'success' as const, generation, payload })),

                catchError((error: unknown) =>

                  of({ kind: 'error' as const, generation, error }),
            ...
            catchError((error: unknown) => of({ kind: 'error' as const, generation, error })),
      ...
      .subscribe((result) => {
        ...
        if (result.kind === 'error') {
          ...
          if (result.generation !== this.loadGeneration) {

            return;

          }

          this.applyResolvedError(result.error, ERROR_MESSAGE);
