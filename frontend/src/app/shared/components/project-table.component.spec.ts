...
describe('ProjectTableComponent', () => {
  ...
  it('should render and filter via the delivery-method dropdown', () => {
    expect(fixture.nativeElement.textContent).toContain('Vorgehensmodell');
    expect(fixture.nativeElement.textContent).toContain('Agil');
    expect(fixture.nativeElement.textContent).toContain('Hybrid');

    const select = fixture.nativeElement.querySelector(
      '.project-table__delivery-select',
    ) as HTMLSelectElement;
    expect(select).toBeTruthy();
    expect(select.options[0].textContent?.trim()).toBe('Alle');
    ...
    const filterService = TestBed.inject(PortfolioFilterService);
    expect(filterService.filters().deliveryMethods).toEqual(['HYBRID']);
    expect(select.value).toBe('HYBRID');
    expect(select.options[select.selectedIndex]?.textContent?.trim()).toBe('Hybrid');
  });

  it('should keep the delivery filter label after selecting Agil', () => {
    const select = fixture.nativeElement.querySelector(
      '.project-table__delivery-select',
    ) as HTMLSelectElement;
    ...
    expect(TestBed.inject(PortfolioFilterService).filters().deliveryMethods).toEqual(['AGILE']);
    expect(select.value).toBe('AGILE');
    expect(select.options[select.selectedIndex]?.textContent?.trim()).toBe('Agil');
  });
