...
class ProjectAiControllerIntegrationTest {
    ...
    @Test
    void approvedContextContainsIsoFactsForAiPipeline() {
        ApprovedProjectContextDto context = approvedProjectDataReader
                .readApprovedContext(KNOWN_PROJECT_ID)
                .orElseThrow();

        long isoFactCount = context.facts().stream()
                .filter(fact -> fact.factId().startsWith("iso."))
                .count();
        org.junit.jupiter.api.Assertions.assertTrue(isoFactCount >= 8);
    }
    ...
    @Test
    void questionAboutIsoBenefitsCanReferenceIsoFacts() throws Exception {
        mockMvc.perform(post("/api/projects/{id}/ai/questions", KNOWN_PROJECT_ID)
                        .with(csrf())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"question\":\"Wie ist der realisierte Nutzen laut ISO-Steuerungsfeldern?\"}"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.aiGenerated").value(true))
                .andExpect(jsonPath("$.answer").exists());
    }
