...
class PortfolioControllerIntegrationTest {
    ...
    void getFilterOptionsReturnsDistinctValues() throws Exception {
        mockMvc.perform(get("/api/portfolio/filters/options"))
                ...
                .andExpect(jsonPath("$.deliveryMethods").isArray())
                .andExpect(jsonPath("$.deliveryMethods", org.hamcrest.Matchers.hasItems("AGILE", "HYBRID", "WATERFALL")));
    }
    ...
    void getPortfolioProjectsReturnsManagementTableFromSeed() throws Exception {
        mockMvc.perform(get("/api/portfolio/projects"))
                ...
                .andExpect(jsonPath("$.projects[0].statusLabel").exists())
                .andExpect(jsonPath("$.projects[0].deliveryMethod").exists())
                .andExpect(jsonPath("$.projects[0].deliveryMethodLabel").exists())
    ...
    @Test
    void getPortfolioProjectsSupportsDeliveryMethodFilter() throws Exception {
        mockMvc.perform(get("/api/portfolio/projects")
                        .param("deliveryMethod", "AGILE"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.empty").value(false))
                .andExpect(jsonPath("$.projects.length()").value(14))
                .andExpect(jsonPath("$.projects[0].deliveryMethod").value("AGILE"))
                .andExpect(jsonPath("$.projects[0].deliveryMethodLabel").value("Agil"));
    }
