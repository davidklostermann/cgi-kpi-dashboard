...
public class GeminiAiModelClient implements AiModelClient {
    ...
    public ProjectAiQuestionResponseDto answer(ApprovedProjectContextDto context, String question) {
        String prompt = """
                Du bist ein freundlicher Projekt-Assistent im Chat. Antworte natürlich auf Begrüßungen, Smalltalk,
                Erklärungen und Rückfragen. Für konkrete Projektfragen nutze ausschließlich die freigegebenen Fakten —
                erfinde keine Kennzahlen.
                Antworte als JSON:
                { "answer": string, "evidenceFactIds": string[], "insufficientEvidence": boolean }.
                Setze insufficientEvidence=true NUR wenn der Nutzer eine konkrete Projektfrage stellt, die sich mit den
                vorhandenen Fakten nicht beantworten lässt. Bei Begrüßungen, allgemeinen Gesprächen oder Erklärungen:
                insufficientEvidence=false und evidenceFactIds=[].
                Bei beantwortbaren Projektfragen: evidenceFactIds aus der Fact-Liste, insufficientEvidence=false.

                Projekt: %s
                Frage: %s
                Fakten:
                %s
                """.formatted(context.projectName(), question, factsAsText(context.facts()));
        ...
        if (insufficient && evidence.isEmpty() && answer.isBlank()) {
            answer = "Dazu liegen keine ausreichend konkreten freigegebenen Projektdaten vor.";
        }
    ...
    public ProjectAiQuestionResponseDto answerPortfolio(ApprovedPortfolioContextDto context, String question) {
        String prompt = """
                Du bist ein freundlicher Portfolio-Assistent im Chat. Antworte natürlich auf Begrüßungen, Smalltalk,
                Erklärungen und Rückfragen. Für konkrete Portfoliofragen nutze ausschließlich die freigegebenen
                Portfolio-Fakten — erfinde keine Kennzahlen und keine projektspezifischen Details außerhalb der Fakten.
                Antworte als JSON:
                { "answer": string, "evidenceFactIds": string[], "insufficientEvidence": boolean }.
                Setze insufficientEvidence=true NUR wenn der Nutzer eine konkrete Portfoliofrage stellt, die sich mit den
                vorhandenen Fakten nicht beantworten lässt. Bei Begrüßungen, allgemeinen Gesprächen oder Erklärungen:
                insufficientEvidence=false und evidenceFactIds=[].
                Bei beantwortbaren Fragen: evidenceFactIds aus der Fact-Liste, insufficientEvidence=false.

                Frage: %s
                Portfolio-Fakten:
                %s
                """.formatted(question, portfolioFactsAsText(context.facts()));
        ...
        String answer = text(root, "answer");
        if (insufficient && evidence.isEmpty() && answer.isBlank()) {
            answer = "Dazu liegen keine ausreichend konkreten freigegebenen Portfolio-Daten vor.";
        }
        return new ProjectAiQuestionResponseDto(
                answer,
                evidence,
