...
public class PortfolioProjectFilter {
    ...
    public List<PortfolioKpiProjectInput> applyProjects(
        ...
        return projects.stream()
                ...
                .filter(project -> matchesDeliveryMethods(project, criteria.deliveryMethods()))
                .toList();
    }
    ...
    private static boolean matchesDeliveryMethods(PortfolioKpiProjectInput project, List<String> deliveryMethods) {
        if (deliveryMethods == null || deliveryMethods.isEmpty()) {
            return true;
        }
        String method = project.deliveryMethod() != null
                ? project.deliveryMethod().trim().toUpperCase(Locale.ROOT)
                : "WATERFALL";
