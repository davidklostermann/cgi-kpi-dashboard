...
public class PortfolioAiAnalysisService {
    ...
    public PortfolioTrendAnalysisResponseDto analyzeTrend(PortfolioFilterCriteria criteria) {
        ...
        try {
            ...
            // Align with project AI graceful degradation (NFR-5): keep detector facts usable
            // when the portfolio LLM call fails (often larger prompt than project analysis).
            PortfolioTrendAnalysisResponseDto fallback = validate(
                    context,
                    new PortfolioTrendAnalysisResponseDto(
                            detected,
                            false,
    ...
    public ProjectAiQuestionResponseDto ask(
            PortfolioFilterCriteria criteria, ProjectAiQuestionRequestDto request) {
        currentUserService.requireAdmin();
        UUID userId = currentUserService.requireUserId();
        ensureEnabled(userId);
        ...
        ApprovedPortfolioContextDto context = contextAssembler.assemble(
                ...
                portfolioTableReader.readTable(safe));
        try {
            ProjectAiQuestionResponseDto raw = aiModelClient.answerPortfolio(context, question);
            return aiEvidenceValidator.validatePortfolioQuestion(context, raw);
        } catch (GeminiTransportException ex) {
            throw AiProviderExceptionMapper.toApiException(
                    ex, "Die Portfolio-Musteranalyse ist derzeit nicht verfügbar.");
