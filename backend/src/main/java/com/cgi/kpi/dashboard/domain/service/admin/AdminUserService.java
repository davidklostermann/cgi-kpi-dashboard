...
public class AdminUserService {
    ...
    public void resetPassword(UUID userId, AdminResetPasswordRequestDto request) {
        ...
        AppUser user = appUserRepository.findById(userId)
                .orElseThrow(() -> new ApiException("USER_NOT_FOUND", "User findById failed", HttpStatus.NOT_FOUND));
    ...
    public void deleteUser(UUID userId) {
        ...
        if (userId.equals(currentUserId)) {
            throw new ApiException("SELF_DELETION_FORBIDDEN", "You cannot delete your own account", HttpStatus.BAD_REQUEST);
        }
        ...
        if (membership.getRole() == WorkspaceRole.ADMIN) {
            long otherAdmins = workspaceMembershipRepository.countActiveMembersWithRoleExcept(WorkspaceRole.ADMIN, userId);
            if (otherAdmins == 0) {
                throw new ApiException("LAST_ADMIN_PROTECTION", "Cannot delete the last active administrator", HttpStatus.BAD_REQUEST);
            }
        }

        aiProviderConfigRepository.deleteByUserId(userId);
        workspaceMembershipRepository.deleteByUserId(userId);
        appUserRepository.delete(user);

        log.info("USER_DELETED: Admin {} deleted user {}", currentUserId, userId);
    }
